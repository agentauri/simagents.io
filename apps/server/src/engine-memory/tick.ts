 ...
 *
 * Purpose: prove the simulation can advance with NO database and NO Redis —
 * only the in-memory store, the real physics constants (CONFIG) and the real
 * pure grid utilities. Agents use a simple rule-based policy (no LLM) so the
 * spike has zero external dependencies.
 *
