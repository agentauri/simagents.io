 ...
 *
 * Browser-local backing store for the simulation engine. It mirrors the
 * relational shape the core tick loop needs (agents, world state, resource
 * spawns, shelters, inventory, events) as plain in-memory collections.
 *
 ...
 * unchanged against memory.
 *
 * Types are imported type-only from `db/schema`; the browser engine never opens
 * an external persistence connection.
 */
